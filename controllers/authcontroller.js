...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (await currentUser.changedPasswordAfter(decodded.iat)) {
    return next(
...
exports.isLoggedIn = async (req, res, next) => {
  ...
  if (req.cookies.jwt) {
    try {
      // Verifying validation of token
      const decodded = await promisify(jwt.verify)(
        req.cookies.jwt,
...
exports.restrictTo = (...roles) => {
  return (req, res, next) => {
    if (!roles.includes(req.user.role)) {
      return next(
        new AppError('you do not access to perform this action', 403),
      );
    }
    next();
  };
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new AppError('There is no user available with this email adress', 404),
    );
  }
  ...
  try {
    ...
    res.status(200).json({
      status: 'success',
      message: 'Token successfully sent via email',
    });
  } catch (err) {
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // 2) Checking if the token is valid and user existence
  if (!user) {
  ...
  // 3) Updating the passwordChangedAt property
  // 4) Logging the user in
  createSendToken(user, 201, res);
});

exports.updateMyPassword = catchAsync(async (req, res, next) => {
  // 1) Getting user from the collection
